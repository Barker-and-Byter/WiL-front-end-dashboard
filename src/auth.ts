    ...
    callbacks: {
        async signIn({ user, account, profile }) {
            ...
            if (account?.provider === "google"){
                console.info(`[AUTH] User succesfully logged in: ${user?.email} via ${account?.provider}`);
            } else if (account?.provider === "github"){
                console.info(`[AUTH] User succesfully logged in: ${profile?.login} via ${account.provider}`);
            } else if (account?.provider === "credentials") {
                console.info(`[AUTH] User succesfully logged in: ${user?.email} via credentials`);
            }
            return true;
        },
    },
